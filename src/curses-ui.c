...
void ui_option_draw(struct ui_option const *option,
                    enum ui_selection_state state, WINDOW *window) {
  int x, y;
  getyx(window, y, x);
...
struct ui_state_change on_key(struct ui_section *section, int key,
                              WINDOW *window) {
...
struct ui_state_change ui_chain_activate(struct ui_option const *option,
                                         WINDOW *window) {
...
struct ui_section make_chain_section(struct ui_section *parent) {
  ...
  static char const input_description[] = "View of manipulate the input chain.";
...
struct ui_state_change ui_table_activate(struct ui_option const *option,
                                         WINDOW *window) {
  ...
  for (bool stop = false; !stop;) {
    ...
    case ui_state_change_tag_change_section:
      ...
      case ui_section_tag_chains:
        ...
        break;
      default:
        dief("unknown ui_section_tag: %d", result.destination_section);
        break;
      }
      break;
